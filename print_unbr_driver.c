...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/05/24 14:31:12 by ehaggon          #+#    #+#             */
...
void	print_unbr_case2(t_argum *p, uintmax_t n, int space)
{
	char c;
...
void	print_unbr_case3(t_argum *p, uintmax_t n, int pads)
{
	process_sp_plus_flag_unbr(p);
	put_nchar('0', pads);
	(!n && !p->pcn && p->flag & _F_PCN) ? 0 : print_uintmax(n);
...
void	print_unbr_case4(t_argum *p, uintmax_t n, int space, int pads)
{
	(!(p->flag & _F_MINUS)) && put_nchar(' ', space);
	process_sp_plus_flag_unbr(p);
	put_nchar('0', pads);
...
void	print_unbr_driver(t_argum *p, uintmax_t n, int space, int pads)
{
	if (!space && !pads)
